...
try {
    ...
    // 장례식장 정보 업데이트
    $stmt = $conn->prepare("UPDATE funeral_hall SET hall_name = ?, tel = ?, addr_sido = ?, addr_sigungu = ?, addr_detail = ?, addr_full = ?, latitude = ?, longitude = ?, representativename = ?, companyno = ?, homepage = ?, businessdate = ?, mortuaycnt = ?, charnelabilitycnt = ?, parkcnt = ?, mealroomyn = ?, superyn = ?, parkyn = ?, waitroomyn = ?, imparyn = ? WHERE hall_id = ?");
    $stmt->bind_param("ssssssddssssiiiiiiiii", $hall_name, $tel, $addr_sido, $addr_sigungu, $addr_detail, $full_address, $latitude, $longitude, $representativename, $companyno, $homepage, $businessdate, $mortuaycnt, $charnelabilitycnt, $parkcnt, $mealroomyn, $superyn, $parkyn, $waitroomyn, $imparyn, $hall_id);
    ...
    if ($stmt->execute()) {
        ...
        error_log("Funeral hall update error: " . $error);
        echo json_encode(['success' => false, 'message' => '장례식장 정보 수정에 실패했습니다: ' . $error]);
    }
    
} catch (Exception $e) {
    error_log($e->getMessage());
    echo json_encode(['success' => false, 'message' => '수정 중 오류가 발생했습니다.']);
    if (isset($conn)) {
        $conn->close();
    }
}
